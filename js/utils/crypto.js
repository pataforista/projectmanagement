 ...
 */
export async function injectWorkspaceSalt(saltB64, saltChecksum = null, iterations = null) {
    ...
    // If iterations changed, update localStorage NOW so unlock() uses the new value
    if (iterations && iterations !== getStoredIterations()) {
        console.log(`[Fortress] Adopting remote iterations: ${iterations}`);
        localStorage.setItem(PBKDF2_ITERATIONS_KEY, String(iterations));
    }
    ...
    if (saltB64 !== currentB64) {
        ...
        try {
            console.log('[Fortress] Frictionless: attempting auto-unlock with new salt/iterations...');
