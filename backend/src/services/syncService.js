export class SyncService {
   ...
   *
   * ✅ FIX 2.1: Added deduplication by entityId to prevent sync_queue amplification
   */
  async processPush(db, userId, deviceId, changes) {
    if (!changes || !Array.isArray(changes)) return [];

    // ✅ FIX 2.1: Deduplicate changes by entityId (keep last occurrence)
    ...
    for (const change of dedupMap.values()) {
      try {
        ...
        // 2. ✅ FIX 2.1: Use UPSERT instead of INSERT to deduplicate in sync_queue
  ...
  async prepareApplyStatement(db, userId, tableName, change) {
    ...
    if (change.action === 'CREATE') {
       ...
       // ✅ FIX 2.2: Validate parent entity ownership on CREATE
